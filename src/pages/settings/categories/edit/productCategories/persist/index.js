...
@connect(selector, (dispatch) => ({
  fetchProductCategory: bindActionCreators(actions.fetchProductCategory, dispatch)
}))
@reduxForm({
...
@Radium
export default class ProductCategoryModal extends Component {
  ...
  async componentWillMount () {
    ...
    // When we want to edit a productCategory, we need to fetch the full version first.
    if (this.props.productCategoryId) {
      const { id, name, locales } = await this.props.fetchProductCategory({ productCategoryId: this.props.productCategoryId });
      const nameCopy = {};
  ...
  render () {
    ...
    return (
      <PersistModal isOpen submitButtonText={edit ? 'Save' : 'Add'} title={edit ? 'Edit Product Category' : 'Add Product Category'} onClose={this.onCloseClick} onSubmit={handleSubmit(this.submit)}>
        <Field
          component={TextInput}
          label='English'
          name='name.en'
