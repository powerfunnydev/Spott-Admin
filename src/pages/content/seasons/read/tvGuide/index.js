...
export default class TvGuideList extends Component {
  ...
  async componentWillMount () {
    await this.props.load(this.props.location.query, this.props.params.seasonId);
  }
  ...
  onClickNewEntry (e) {
    e.preventDefault();
    const { seasonId, seriesEntryId } = this.props.params;
  ...
  render () {
    const { isSelected, pageCount, selectAllCheckboxes,
      selectCheckbox, totalResultCount, tvGuideEntries, location: { query: { tvGuidePage,
        tvGuideSortField, tvGuideSortDirection } }, params: { seriesEntryId, seasonId } } = this.props;
                ...
                {tvGuideEntries.get('data').map((tvGuideEntry, index) => {
                  return (
                    <Row index={index} isFirst={index % numberOfRows === 0} key={index} >
                      ...
                      <DropdownCel>
                        <Dropdown
                          elementShown={<div key={0} style={[ dropdownStyles.clickable, dropdownStyles.option, dropdownStyles.borderLeft ]} onClick={() => { this.props.routerPushWithReturnTo(`content/series/read/${seriesEntryId}/seasons/read/${seasonId}/tv-guide/edit/${tvGuideEntry.get('id')}`); }}>Edit</div>}>
