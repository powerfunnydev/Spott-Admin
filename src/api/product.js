...
export async function fetchProduct (baseUrl, authenticationToken, locale, { characterId }) {
...
export async function deleteProduct (baseUrl, authenticationToken, locale, { characterId }) {
  await del(authenticationToken, locale, `${baseUrl}/v004/media/characters/${characterId}`);
}

export async function deleteProducts (baseUrl, authenticationToken, locale, { characterIds }) {
  for (const characterId of characterIds) {
    await deleteProduct(baseUrl, authenticationToken, locale, { characterId });
  }
}
